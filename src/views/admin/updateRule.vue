<template>
    <div v-loading="state.skeleton">
                    ...
                    <el-form ref="form" :model="state.form" :rules="state.rules" label-width="80px">
                        <el-form-item label="上级" prop="pid">
                            <el-select v-model="state.form.pid" clearable placeholder="选择上级">
                                <el-option v-for="item in state.oneRuleList" :key="item.id" :label="item.name" :value="item.id" :disabled="item.id == state.form.id" />
                            </el-select>
                        </el-form-item>
        ...
        <fixed-action-bar>
            <el-button v-auth="['addRule']" type="primary" :loading="!state.isClick" @click="onSubmit">更新</el-button>
        </fixed-action-bar>
    </div>
</template>
...
let onSubmit = () => {
    form.value.validate(valid => {
        if (valid) {
            if (state.isClick) {
                state.isClick = false
                api.post('admin/updateRule', state.form).then(res => {
                    if (res.code == 1) {
                        ElMessage.success({
                            message: res.msg,
                            onClose() {
